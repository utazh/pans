...
class PinnedStagingPool:
    """Two bounded host slabs; only a completed H2D event permits reuse.

    The pool retains the allocation independently of prefetch handles. Waiting
    for a slab waits for the copy event, never for its consumer on the main
    thread, so speculative P8 queues cannot exhaust the pool and deadlock.
    """
    def __init__(self, slab_bytes=4 * 1024 * 1024, slots=2):
        self.slab_bytes = int(slab_bytes)
        self.slots = [dict(buffer=None, event=None, leased=False) for _ in range(slots)]
        self.condition = threading.Condition()
        self.allocations = self.reuses = self.waits = self.h2d_copies = 0
        self.h2d_bytes = self.peak_bytes = 0

    def acquire(self, dtype):
        with self.condition:
            while True:
                available = [s for s in self.slots if not s["leased"]]
                ready = [s for s in available
                         if s["event"] is None or s["event"].query()]
                if ready:
                    slot = ready[0]
                    break
                if available:
                    self.waits += 1
                    available[0]["event"].synchronize()
                else:
                    self.condition.wait()
            slot["event"] = None
            tensor = slot["buffer"]
            if tensor is None or tensor.dtype != dtype:
                size = self.slab_bytes // torch.empty(0, dtype=dtype).element_size()
                # Do not silently substitute pageable RAM on a CUDA machine.
                tensor = torch.empty(size, dtype=dtype,
                                     pin_memory=torch.cuda.is_available())
                slot["buffer"] = tensor
                self.allocations += 1
                self.peak_bytes = max(self.peak_bytes, sum(
                    s["buffer"].numel() * s["buffer"].element_size()
                    for s in self.slots if s["buffer"] is not None))
            else:
                self.reuses += 1
            slot["leased"] = True
            return slot, tensor
    ...
    def metrics(self):
        with self.condition:
            return dict(capacity_bytes=self.slab_bytes * len(self.slots),
                        allocated_peak_bytes=self.peak_bytes,
                        allocations=self.allocations, reuses=self.reuses,
                        waits=self.waits, h2d_copies=self.h2d_copies,
                        h2d_bytes=self.h2d_bytes)

_STAGING_POOL = PinnedStagingPool()


def _gather_host_parts(parts, destination):
    """Gather retained read-only sources directly into an existing pinned slab."""
    if destination.dtype == torch.bfloat16:
        torch.cat(parts, dim=0, out=destination)
    else:
        np.concatenate(parts, axis=0, out=destination.numpy())
...
class PrefixKVLayer():
    ...
    def _batch_copy_chunk_sources(self, target, chunk_indices, counts, want_key):
        """Batch by residency, restoring exact output order with index_copy_.

        CPU/disk chunks share bounded pinned slabs even across intervening GPU
        chunks. GPU chunks use batched cat instead of one D2D per chunk. Index
        staging lives inside the same 8 MiB pool, not in an uncounted allocation.
        """
        pool = _STAGING_POOL
        width = int(np.prod(target.shape[1:]))
        row_bytes = target.element_size() * width
        capacity = pool.slab_bytes // (row_bytes + 8)
        capacity -= capacity % 4  # Keep the trailing int64 index area aligned.
        if capacity < self.chunk_size:
            raise ValueError("staging slab must hold one KV chunk plus its indices")
        index_offset = (capacity * row_bytes + 7) // 8 * 8
        assert index_offset + capacity * 8 <= pool.slab_bytes
        offsets = np.arange(self.chunk_size, dtype=np.int64)
        slot = slab = host_array = host_index = None
        used = out_pos = 0
        host_starts, host_counts, host_parts = [], [], []
        gpu_parts, gpu_starts, gpu_counts = [], [], []
        gpu_tokens = 0

        def finish_slot(lease):
            event = None
            if target.device.type == "cuda":
                event = torch.cuda.Event()
                event.record(torch.cuda.current_stream(target.device))
            pool.release(lease, event)

        def positions(starts, takes):
            return (np.asarray(starts, dtype=np.int64)[:, None] + offsets)[
                offsets < np.asarray(takes, dtype=np.int64)[:, None]]

        def flush_host():
            nonlocal slot, slab, host_array, host_index, used
            if slot is None:
                return
            try:
                # NumPy performs the complete host gather in one native call.
                # Views retain CPU tensors/memmaps until the gather completes.
                _gather_host_parts(host_parts, slab[:used])
                contiguous = host_starts[-1] + host_counts[-1] - host_starts[0] == used
                if contiguous:
                    target[host_starts[0]:host_starts[0] + used].copy_(slab[:used], non_blocking=True)
                    pool.h2d_copies += 1
                    pool.h2d_bytes += used * row_bytes
                else:
                    host_index[:used].numpy()[:] = positions(host_starts, host_counts)
                    packed = torch.empty((used, *target.shape[1:]), dtype=target.dtype, device=target.device)
                    lookup = torch.empty(used, dtype=torch.long, device=target.device)
                    packed.copy_(slab[:used], non_blocking=True)
                    lookup.copy_(host_index[:used], non_blocking=True)
                    target.index_copy_(0, lookup, packed)
                    pool.h2d_copies += 2
                    pool.h2d_bytes += used * (row_bytes + 8)
            finally:
                finish_slot(slot)
                slot = slab = host_array = host_index = None
                used = 0
                host_starts.clear(); host_counts.clear(); host_parts.clear()

        def flush_gpu():
            nonlocal gpu_tokens
            if not gpu_parts:
                return
            stream = torch.cuda.current_stream(target.device)
            for part in gpu_parts:
                # Protect immutable source storage if cache eviction follows.
                part.record_stream(stream)
            contiguous = gpu_starts[-1] + gpu_counts[-1] - gpu_starts[0] == gpu_tokens
            if contiguous:
                out = target[gpu_starts[0]:gpu_starts[0] + gpu_tokens]
                if len(gpu_parts) == 1:
                    out.copy_(gpu_parts[0], non_blocking=True)
                else:
                    torch.cat(gpu_parts, dim=0, out=out)
            else:
                lease, flat = pool.acquire(self.torch_dtype)
                try:
                    indices = flat.view(torch.uint8)[:gpu_tokens * 8].view(torch.int64)
                    indices.numpy()[:] = positions(gpu_starts, gpu_counts)
                    lookup = torch.empty(gpu_tokens, dtype=torch.long, device=target.device)
                    lookup.copy_(indices, non_blocking=True)
                    packed = torch.cat(gpu_parts, dim=0)
                    target.index_copy_(0, lookup, packed)
                    pool.h2d_copies += 1
                    pool.h2d_bytes += gpu_tokens * 8
                finally:
                    finish_slot(lease)
            gpu_parts.clear(); gpu_starts.clear(); gpu_counts.clear()
            gpu_tokens = 0

        try:
            for chunk_index, take in zip(chunk_indices, counts):
                if take <= 0:
                    continue
                chunk = ((self.key_tokens if want_key else self.value_tokens)[chunk_index]
                         if self.disk_type == 'KV_Division' else self.tokens[chunk_index])
                source = self._chunk_source_readonly(chunk, want_key=want_key)
                if source.device.type == 'cuda':
                    if gpu_tokens + take > pool.slab_bytes // 8:
                        flush_gpu()
                    gpu_parts.append(source[:take])
                    gpu_starts.append(out_pos); gpu_counts.append(take)
                    gpu_tokens += take
                else:
                    if used + take > capacity:
                        flush_host()
                    if slot is None:
                        slot, flat = pool.acquire(self.torch_dtype)
                        slab = flat[:capacity * width].view(capacity, *target.shape[1:])
                        host_index = flat.view(torch.uint8)[index_offset:index_offset + capacity * 8].view(torch.int64)
                        host_array = slab.numpy() if slab.dtype != torch.bfloat16 else None
                    part = source if take == source.shape[0] else source[:take]
                    host_parts.append(part.numpy() if host_array is not None else part)
                    host_starts.append(out_pos); host_counts.append(take)
                    used += take
                out_pos += take
            flush_host()
            flush_gpu()
            ...
            if slot is not None:
                # Preserve already-enqueued copies, even on a later read failure.
                finish_slot(slot)

    def _prefetch_chunks_direct_pinned(self, chunk_ids, k_v_num_tokens, need_key, need_value):
        indices = chunk_ids.detach().cpu().numpy().astype(np.int64, copy=False)
        starts = indices * self.chunk_size
        counts_array = np.minimum(self.chunk_size, np.maximum(0, int(k_v_num_tokens) - starts))
        counts = counts_array.tolist()
        chunk_indices = indices.tolist()
        total_tokens = int(counts_array.sum())
        shape = (total_tokens, *self.shape[2:])
        ...
        offsets = np.arange(self.chunk_size, dtype=np.int64)
        tokenids_cpu = torch.from_numpy((starts[:, None] + offsets)[offsets < counts_array[:, None]])
        if need_key:
            self._batch_copy_chunk_sources(key_buffer, chunk_indices, counts, True)
        if need_value:
            self._batch_copy_chunk_sources(value_buffer, chunk_indices, counts, False)
        # The pool owns staging allocations until their copy events complete.
        return key_buffer, value_buffer, tokenids_cpu.to(chunk_ids.device), []
    ...
    def prefetch_by_chunk(self,pos_id=None,need_key=True,need_value=True,k_v_num_tokens=None,time_budget=0.0):
        ...
        if len(pos_id.shape) == 1:
            ...
            pos2chunk = pos_id // self.chunk_size
            chunk_values = pos2chunk.detach().cpu().numpy()
            _, first_indices = np.unique(chunk_values, return_index=True)
            ordered_chunk_ids = chunk_values[np.sort(first_indices)].tolist()
            ...
            if time_budget > 0.0:
                ...
                for chunk_offset, chunk_index in enumerate(ordered_chunk_ids):
                    est_time = 0.0
