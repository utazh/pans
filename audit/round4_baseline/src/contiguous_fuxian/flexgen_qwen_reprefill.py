...
def runtime_variant(
    *,
    ...
    promixed_reuse_mode: str = "adaptive",
    ...
    if promixed_gqa_selection:
        variant += (
            f"+promixed-gqa-{promixed_reuse_mode}-p{impress_selection_period_size}"
            "+round1-coverage-zero-off"
        )
    ...
    if loader.method == "as_h2o_lru":
        ...
        if promixed_policy is not None:
            ...
            leader_decision = select_promixed_gqa_blocks(
                ...
                ranked_blocks=prepared_block_scores.ranked_blocks,
...
def flexgen_sparse_decoder_logits(
    *,
    ...
    # Local to this decoder call: masks are read-only and never cross requests.
    mask_cache = {}
    ...
    for layer_index, decoder_layer in enumerate(core.layers[: core.config.num_hidden_layers]):
        ...
        mask_key = (query_tokens, cache.get_seq_length(layer_index),
                    hidden_states.dtype, hidden_states.device)
        attention_mask = mask_cache.get(mask_key)
        if attention_mask is None:
            attention_mask = _layer_causal_mask(
                query_tokens=mask_key[0], past_tokens=mask_key[1],
                dtype=mask_key[2], device=mask_key[3],
            )
            if len(mask_cache) >= 8:
                mask_cache.pop(next(iter(mask_cache)))
            mask_cache[mask_key] = attention_mask
...
def greedy_flexgen_completion(
    *,
    ...
    profile_nvtx = os.environ.get("PRISM_PROFILE_NVTX") == "1"
    if profile_nvtx:
        torch.cuda.nvtx.range_push("TTFT/logits_ready")
    ...
    if profile_nvtx:
        ...
        torch.cuda.nvtx.mark("logits_ready")
    ...
    if profile_nvtx:
        torch.cuda.nvtx.mark("first_token_ready")
    label_cache_data = (
    ...
    if profile_nvtx:
        torch.cuda.nvtx.mark("response_ready")
    ...
    if profile_nvtx:
        torch.cuda.nvtx.mark("evaluation_ready")
...
def validate_bundle_prefix_identity(rows, *, store_root, tokenizer):
    """Bind actual request prefixes to the stored token identity before timing."""
    from .sparse_qwen_reprefill import _task_prefixes, _token_hash
    ...
    for task, prefix in _task_prefixes(rows).items():
        ...
        token_hash = _token_hash(ids)
        if len(ids) != info.prefix_tokens or token_hash != info.token_hash:
            raise ValueError(f"request bundle prefix identity mismatch for {task!r}")
        identities[task] = token_hash
    return identities
...
def run_flexgen_reprefill(
    *,
    ...
    if exact_layer_block_budget and (
        ...
        raise ValueError(
            "exact layer block budgets require online IMPRESS "
    ...
    if promixed_reuse_mode not in {"adaptive", "fixed"}:
        raise ValueError("ProMixed reuse mode must be adaptive or fixed")
    if promixed_reuse_mode == "fixed" and not promixed_gqa_selection:
        raise ValueError("Fixed ProMixed reuse requires --promixed-gqa-selection")
    promixed_policy: dict[str, float | bool | str] | None = None
    if promixed_gqa_selection:
        if (
            not online_selection
            or method != "impress"
        ...
        promixed_policy = {
            "reuse_mode": promixed_reuse_mode,
    ...
    validate_bundle_prefix_identity(rows, store_root=store_root, tokenizer=tokenizer)
    budget_profile = None
    ...
    if flexgen_config.selector_index_dir is not None:
        from .quantized_key_index import QuantizedKeyIndex, model_identity
        QuantizedKeyIndex(flexgen_config.selector_index_dir).validate_model(model_identity(model_path))
    ...
    def execute(row: dict[str, Any]) -> dict[str, Any]:
        ...
        loader = store.new_layer_loader(
            ...
            layer_keep_ratios=(
                ...
                else ((plan_keep_ratio,) * info.layers if exact_layer_block_budget else None)
            ),
            layer_keep_blocks=(
                allocate_exact_layer_blocks(
                    (budget_profile.layer_ratios if budget_profile is not None
                     else (plan_keep_ratio,) * info.layers),
    ...
    try:
        ...
        progress_path = output / "requests.progress.jsonl"
        with progress_path.open("x", encoding="utf-8") as progress:
            for row in rows:
                ...
                if len(scored) % 25 == 0 or len(scored) == len(rows):
                    print(f"completed {len(scored)}/{len(rows)} requests", flush=True)
    finally:
        store.close()
    ...
    summary: dict[str, Any] = {
        ...
        "runtime": {
            ...
            "runtime_variant": runtime_variant(
                ...
                promixed_reuse_mode=promixed_reuse_mode,
                defer_cache_score_updates=defer_cache_score_updates,
            ...
            "exact_layer_block_budget": exact_layer_block_budget,
            "layer_budget_mode": "profile" if budget_profile is not None else "uniform",
            "selection_policy_version": "round1-coverage-zero-off",
            "promixed_reuse_mode": promixed_reuse_mode if promixed_gqa_selection else None,
            "reuse_diagnostics_still_computed": bool(promixed_gqa_selection),
            "layer_keep_ratios": (
                list(budget_profile.layer_ratios)
                if budget_profile is not None
                else ([plan_keep_ratio] * int(model.config.num_hidden_layers)
                      if exact_layer_block_budget else None)
            ),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "reuse selections according to --promixed-reuse-mode."
        ),
    )
    parser.add_argument(
        "--promixed-reuse-mode", choices=("adaptive", "fixed"), default="adaptive",
        help="Fixed uses exactly the configured selection period; P1 selects every layer.",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Disable group reservation when uncertainty is below P4 threshold; "
            "round-1 zero coverage means no reservation, not a minimum quota."
        ),
    )
    parser.add_argument(
    ...
    summary = run_flexgen_reprefill(
        ...
        promixed_reuse_mode=args.promixed_reuse_mode,
