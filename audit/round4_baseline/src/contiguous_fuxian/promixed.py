...
class PromixedSelectionDecision:
    """A fixed-budget selection plus the configured cross-layer reuse horizon."""
...
def select_promixed_gqa_blocks(
    ...
    *,
    keep_blocks: int,
    ranked_blocks: Sequence[Sequence[int]] | None = None,
    coverage_fraction: float = 0.5,
    max_period: int = 8,
    reuse_mode: Literal["adaptive", "fixed"] = "adaptive",
    ...
    choose a P1/P2/P4/P8 reuse horizon only in ``reuse_mode="adaptive"``.
    Fixed mode always returns ``max_period``; use 1 for per-layer selection.
    Diagnostics are still computed in fixed mode and must be charged to timing.

    Round-1 coverage semantics: an effective coverage fraction of zero disables
    the reservation stage completely. Positive fractions retain the historical
    minimum-group-reservation behavior. This intentionally changes historical
    zero-coverage/adaptive-zero selections; version experimental results.
    """
    ...
    if reuse_mode not in {"adaptive", "fixed"}:
        raise ValueError("ProMixed reuse mode must be adaptive or fixed")
    if isinstance(max_period, bool) or not isinstance(max_period, Integral):
        raise ValueError("ProMixed max period must be an integer")
    ...
    if ranked_blocks is None:
        ...
        rankings = tuple(tuple(row) for row in ranked_blocks)
        if len(rankings) != head_count:
            raise ValueError("prepared rankings must match score rows")
        for row, ranking in zip(rows, rankings):
            if (len(ranking) != block_count
                or any(isinstance(b, bool) or not isinstance(b, Integral) for b in ranking)
                or set(ranking) != set(range(block_count))):
                raise ValueError("prepared ranking must be a block permutation")
            if any((-row[left], left) > (-row[right], right)
                   for left, right in zip(ranking, ranking[1:])):
                raise ValueError("prepared ranking does not match score order")
    ...
    adaptive_period = _adaptive_period(
        uncertainty,
    ...
    period = int(max_period) if reuse_mode == "fixed" else adaptive_period
    ...
    coverage_target = (
        0
        if effective_coverage_fraction == 0.0
        else min(
            keep_blocks,
            max(
