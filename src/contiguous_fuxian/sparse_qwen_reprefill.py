...
def _layer_causal_mask(*, query_tokens: int, past_tokens: int, dtype, device):
    ...
    # The first past_tokens columns and the query's lower triangle are visible.
    # triu preserves the exact 0 / finfo.min values without one launch per row.
    return mask.triu(diagonal=past_tokens + 1)
