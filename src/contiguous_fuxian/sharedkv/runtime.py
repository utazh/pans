...
class RequestState:
    ...
    tail_lengths: dict = field(default_factory=dict)
...
def decoder_logits(*, model, input_ids, cache, position_start, loader,
    ...
    for lid,layer in enumerate(core.layers):
        ...
        state.tail_lengths[lid] = max(state.tail_lengths.get(lid,0),int(tk.shape[2]))
...
class SharedRuntime:
    def __init__(self, *, mode, backend, adapter_dir=None, stats_path=None, allow_diagnostic=False):
        if mode not in ("identity","shared") or backend not in ("reference","flash2","vllm2"):
    ...
    def _prepare(self,model,loader,period):
        ...
        if self.mode=="shared":
            ...
            if bundle.get("diagnostic_only",False) and not self.allow_diagnostic:
                raise ValueError("Locally rejected candidate bundle: use --allow-diagnostic only for research ablations")
        ...
        if self.backend in ("flash2","vllm2"):
            ...
            attention_with_lse(q,k,k,backend=self.backend,causal=True)
            ...
            try:
                ...
                if self.stats_path:
                    ...
                    stats = {"query_hash":digest,"task":loader._info.task,"mode":self.mode,
                        ...
                        "private_tail_lengths":state.tail_lengths,
                        "query_tokens":len(kw["query_token_ids"]),
                        "requested_max_tokens":kw["max_tokens"],
                        "prefix_storage_ptrs_by_layer":[[state.bank[s][0].data_ptr(),state.bank[s][1].data_ptr()] for s in sources],
