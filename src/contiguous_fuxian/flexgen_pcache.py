...
def prefetch_source_tensor_tokens(
    ...
    *,
    ...
    chunks = np.unique(np.asarray(token_ids, dtype=np.int64) // chunk_size)
    return _prefetch_source_chunk_counts(
        pcache, prefix_id=prefix_id, layer=layer, chunks=chunks,
        chunk_size=chunk_size, prefix_tokens=prefix_tokens,
    )


def _prefetch_source_chunk_counts(
    pcache, *, prefix_id, layer, chunks, chunk_size, prefix_tokens,
):
    """Reuse chunk metadata; take a fresh residency snapshot at consumption."""
    cache_layer = pcache.cache[prefix_id].layers[layer]
    chunk_count = int(cache_layer.chunk_num)
    if chunks.size and (chunks[0] < 0 or chunks[-1] >= chunk_count):
        raise ValueError(f"physical chunk is outside layer {layer}")
    tokens = np.minimum(chunk_size, np.maximum(0, prefix_tokens - chunks * chunk_size))
    indices = np.concatenate((chunks, chunks + chunk_count))
    weights = np.tile(tokens, 2)
    devices = np.asarray(cache_layer.device_map)[indices].astype(str, copy=False)
    gpu = np.char.startswith(devices, "cuda")
    cpu, disk = devices == "cpu", devices == "disk"
    if not np.all(gpu | cpu | disk):
        raise RuntimeError(f"unsupported HyperInfer cache device {devices[~(gpu | cpu | disk)][0]!r}")
    return {"gpu": int(weights[gpu].sum()), "cpu": int(weights[cpu].sum()),
            "disk": int(weights[disk].sum())}
...
def _ordered_chunk_counts(selected_tokens, prefix_tokens, chunk_size, reorder=None):
    """Batch metadata only; preserve the original cache update/eviction order."""
    import numpy as np
    tokens = np.unique(np.asarray(selected_tokens, dtype=np.int64))
    if tokens.size and (tokens[0] < 0 or tokens[-1] >= prefix_tokens):
        raise ValueError("selected token is outside the prefix")
    physical = np.asarray(reorder, dtype=np.int64)[tokens] if reorder is not None else tokens
    chunks, first, counts = np.unique(physical // chunk_size,
                                     return_index=True, return_counts=True)
    order = np.argsort(first)
    return dict(zip(chunks[order].tolist(), counts[order].tolist()))
...
class FlexGenPcacheStore:
    ...
    def new_layer_loader(
        ...
        *,
        ...
        promixed_policy: Mapping[str, float | bool | str] | None = None,
    ...
    def update_impress_layer_scores(
        ...
        *,
        ...
        counts = _ordered_chunk_counts(
            selected_tokens, info.prefix_tokens, self.config.chunk_size,
            task_reorder[layer] if task_reorder is not None else None,
        )
        if not counts:
...
class FlexGenLayerLoader:
        ...
        *,
        ...
        if promixed_policy is not None:
            ...
            normalized_promixed_policy = {
                str(key): (
                    str(value)
                    if str(key) == "reuse_mode"
                    else (bool(value) if str(key) == "adaptive_coverage" else float(value))
                )
                for key, value in promixed_policy.items()
            }
            if normalized_promixed_policy.get("reuse_mode", "adaptive") not in {"adaptive", "fixed"}:
                raise ValueError("ProMixed reuse mode must be adaptive or fixed")
        else:
        ...
        self._physical_to_logical = (
            [np.array(row, dtype=np.int64, copy=True) for row in physical_to_logical]
            if physical_to_logical is not None
            else None
        )
        self._logical_to_physical = (
            [np.array(row, dtype=np.int64, copy=True) for row in logical_to_physical]
            if logical_to_physical is not None
            else None
        )
        self._selected_storage_by_layer = {}
    ...
    def _storage_positions(self, layer: int, logical_positions: Sequence[int]) -> list[int]:
        ...
        cached = getattr(self, "_selected_storage_by_layer", {}).get(layer)
        if cached is not None and cached[0] is logical_positions:
            return list(cached[1])
        positions = np.asarray(logical_positions, dtype=np.int64)
        if self._logical_to_physical is None:
            return positions.tolist()
        mapping = np.asarray(self._logical_to_physical[layer], dtype=np.int64)
        return mapping[positions].tolist()
    ...
    def _logical_segment(self, layer: int, key: Any, value: Any, physical_ids: Any):
        ...
        # Constructor-owned NumPy rows share storage with CPU tensor views.
    ...
    def configure_impress_layer(
        ...
        *,
        ...
        self._selected_storage_by_layer[layer] = (selected, tuple(storage_positions))
    ...
    def schedule_impress_missing(self, layer: int) -> None:
        ...
        selected_storage = self._storage_positions(layer, selected)
        missing = [
            token for token, physical in zip(selected, selected_storage)
            if physical // self._config.chunk_size not in covered_chunks
        ]
        self._inter_period_missing_tokens += len(missing)
    ...
    def _resolve_entry(self, layer: int, entry: tuple[Any, float]) -> tuple[Any, Any, Any]:
        ...
        self._record_physical_access(layer, physical_ids.numpy())
    ...
    def _record_physical_access(self, layer: int, physical_ids: Sequence[int]) -> None:
        chunks = np.unique(np.asarray(physical_ids, dtype=np.int64) // self._config.chunk_size)
        source_counts = _prefetch_source_chunk_counts(
            self._pcache, prefix_id=self._prefix_id, layer=layer, chunks=chunks,
            chunk_size=self._config.chunk_size, prefix_tokens=self._info.prefix_tokens,
        )
        for source, count in source_counts.items():
            self._source_tensor_tokens[source] += count
        self._physical_tokens += len(physical_ids)
        self._physical_chunks.update(
            (layer, chunk) for chunk in chunks.tolist()
        )
